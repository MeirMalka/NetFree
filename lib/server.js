...
function Server( opts, requestListener ) {
	...
	this.filters = filters({ pathName: path.join( __dirname , '../filters') , connection: this.options.connection });
	...
	this.on('connect', function( request ,  socket , head) {
		...
		if(0){
			...
			certs.genCert( request.host  , function( cert ){
						...
						console.log( "cert for " + request.host );
						...
						proxySslServer.on('request' , function( proxyRequest , response ){
								...
								_this.emit('request' , proxyRequest , response ); //handleRequest();
						});
