...
Filters.prototype.applyFilter = function( type , sess ){
    ...
    for(var i in this.listFilters)
    	sess.listFilters[i] = this.listFilters[i];
	
	function applyFilter()
	{
		sess.type = type;
		var filterName = sess.currentFilter.name ;
		
		if( !filterName || !_this.filters.hasOwnProperty(filterName)) sess.currentFilter =  this.filters[_this.default];
		
		var on = false;
		switch (type) {
			case "request":
				on = _this.filters[sess.currentFilter.name].onRequest;
				break;
			case "response":
				on = _this.filters[sess.currentFilter.name].onResponse;
				break;
	...
	sess.next = function(){
		sess.currentFilter = sess.listFilters.shift();
		if(!sess.currentFilter)
		{
			orginalNext();
			
		}else{
			if("string" == typeof sess.currentFilter) sess.currentFilter = { name: sess.currentFilter };
			applyFilter();
		}
	};
	
	sess.applyFilter = function( name ){
		sess.currentFilter = { name: name };
		applyFilter();
	};
...
Filters.prototype._watch = function(){
	var _this = this;
	var fileReload = path.join(this.pathName , ".reload");
